...
contract simpleVote {
    ...
    mapping(bytes32 =>  mapping ( address => uint )) private received_count_me;
    ...
    function vote( bytes32 candidate, uint count ) public {
        ...
        received_count_me[candidate][msg.sender]+= count;
        tickets[msg.sender] -= count;
    ...
    function getTicketsReceivebyme() view public returns (uint[]) {
        ...
        for(uint i = 0; i < candidateNames.length; i++) {
            ...
            temp.push(received_count_me[candidateNames[i]][msg.sender]);
